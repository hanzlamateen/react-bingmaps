	...
	_createClass(ReactBingmaps, [{
		...
		key: 'setInfoboxesWithPushPins',
            ...
            value: function getClusteredPinsInfo(cluster) {
                ...
                Microsoft.Maps.Events.addHandler(cluster, "mouseover", this.clusterClicked);
            }
        }, {
            key: 'clusterClicked',
            value: function clusterClicked(e) {
                let map = this.mapRef;
                //Create a title for the cluster.
                this.showClusterInfobox(e.target);
            }, {
            key: 'showClusterInfobox',
            value: function showClusterInfobox(cluster) {
                let description = [];
                if (cluster.containedPushpins) {
                    ...
                    for (var i = 0; i < cluster.containedPushpins.length; i++) {
                        description.push('<li>', cluster.containedPushpins[i].getTitle(), '</li>');
                    }
