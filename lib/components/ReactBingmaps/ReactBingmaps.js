...
var ReactBingmaps = function (_Component) {
	...
	_createClass(ReactBingmaps, [{
		...
		key: 'setInfoboxesWithPushPins',
                ...
                Microsoft.Maps.loadModule("Microsoft.Maps.Clustering", function () {
                    ...
                    if (map[mapReference] && infoboxesWithPushPins) {
                        ...
                        if (infoboxesWithPushPins) {
                            //Array of all pushpin locations to property set zoom
                            var locs = [];
                    ...
                    //Create a ClusterLayer and add it to the map.
                    let clusterLayer = new Microsoft.Maps.ClusterLayer(clusterLayerArr, { clusteredPinCallback: that.getClusteredPinsInfo.bind(this, mapReference)});
                    map[mapReference].layers.insert(clusterLayer);

                    //Set the zoom to encompass all pins
                    map[mapReference].setView({
            ...
            value: function getClusteredPinsInfo(cluster, mapReference) {
                Microsoft.Maps.Events.addHandler(cluster, "click", this.props.clusterClickCallback);
                Microsoft.Maps.Events.addHandler(cluster, "mouseover", this.showClusterInfobox.bind(this, mapReference));
            }
        }, {
            key: 'showClusterInfobox',
            value: function showClusterInfobox(e) {
                let description = [];
                if (e.target.containedPushpins) {
                    //Create a list of all pushpins that are in the cluster.
                    description.push('<div style="max-height:75px;overflow-y:auto;"><ul>');
                    for (var i = 0; i < e.target.containedPushpins.length; i++) {
                        description.push('<li>', e.target.containedPushpins[i].getTitle(), '</li>');
                    }
                    description.push('</ul></div>');
                }
                let infobox = new Microsoft.Maps.Infobox(map[mapReference].getCenter(), { visible: false })
