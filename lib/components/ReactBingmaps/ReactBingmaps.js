...
var ReactBingmaps = function (_Component) {
	...
	function ReactBingmaps(props) {
        ...
        this.state = { clusterCallback: props.clusterClickCallback };

        this.getClusteredPinsInfo = this.getClusteredPinsInfo.bind(this);
    ...
    _createClass(ReactBingmaps, [{
        ...
        value: function componentWillReceiveProps(nextProps) {
            ...
            if ((this.props.center && nextProps.center) && (this.props.center.join() !== nextProps.center.join())) {
            ...
            if (this.props.infoboxesWithPushPins !== nextProps.infoboxesWithPushPins) {
                this.setState({ clusterCallback: nextProps.clusterClickCallback },
			...
			if (this.props.autoSuggest !== nextProps.autoSuggest) {
				this.setAutoSuggest(nextProps.autoSuggest, mapReference);
			}
		...
		value: function reactBingmaps(props, Microsoft) {
            var bingmapKey = props.bingmapKey,
                ...
                clusterClickCallback = props.clusterClickCallback;
			...
			if (bingmapKey && Microsoft) {
                ...
                this.setState({ clusterCallback: this.props.clusterClickCallback },
                    () => {
				...
				this.setAutoSuggest(autoSuggest, mapReference);
		...
		key: 'setPushPins',
            value: function setPushPins(pushPins, mapReference) {
                ...
                Microsoft.Maps.loadModule("Microsoft.Maps.Clustering", function () {
                        ...
                        for (var pushPinIndex = 0; pushPinIndex < pushPins.length; pushPinIndex++) {
                            if (pushPins[pushPinIndex].location && pushPins[pushPinIndex].location[0] && pushPins[pushPinIndex].location[1]) {
                                ...
                                if (pushPins[pushPinIndex].addHandler) {
                                    ...
                                    }.bind(that, pushPins[pushPinIndex].addHandler.callback, pushPins[pushPinIndex].addHandler.callbackData));
                                }
                            }
                        }
                    }
                    ...
                    let clusterLayer = new Microsoft.Maps.ClusterLayer(clusterLayerArr);
		...
		key: 'setInfoboxesWithPushPins',
            value: function setInfoboxesWithPushPins(infoboxesWithPushPins, infoboxCreateType, mapReference) {
                let that = this;
                //Keeps track of pushPin locations so that the view can be properly set
                var locs = [];
                if(!Microsoft) {
                    return;
                }
                Microsoft.Maps.loadModule("Microsoft.Maps.Clustering", function () {
                    ...
                    if (map[mapReference] && infoboxesWithPushPins) {
                        ...
                        //Clear all pushpins
						map[mapReference].entities.clear();
                                ...
                                if (infoboxesWithPushPins[infoboxWithPushPinIndex].location) {
                                    ...
                                    if (infoboxesWithPushPins[infoboxWithPushPinIndex].infoboxAddHandler) {
                                        Microsoft.Maps.Events.addHandler(infobox[infoboxCreateType][infoboxWithPushPinIndex], infoboxesWithPushPins[infoboxWithPushPinIndex].infoboxAddHandler.type, function (callback, data) {
                                            that.MakeCallback(callback, data);
                                        }.bind(that, infoboxesWithPushPins[infoboxWithPushPinIndex].infoboxAddHandler.callback, infoboxesWithPushPins[infoboxWithPushPinIndex].infoboxAddHandler.callbackData));
                                    }

                                    //Set Pushpin				
                                    var _pushpin2 = new Microsoft.Maps.Pushpin(location, pushPinOption);
                                    clusterLayerArr.push(_pushpin2);
                                    locs.push(location);
                                    ...
                                    if (infoboxesWithPushPins[infoboxWithPushPinIndex].pushPinAddHandler) {
                                        Microsoft.Maps.Events.addHandler(_pushpin2, infoboxesWithPushPins[infoboxWithPushPinIndex].pushPinAddHandler.type, function (callback, data) {
                                            that.MakeCallback(callback, data);
                                        }.bind(that, infoboxesWithPushPins[infoboxWithPushPinIndex].pushPinAddHandler.callback, infoboxesWithPushPins[infoboxWithPushPinIndex].pushPinAddHandler.callbackData));
                                    }

                                    //Set InfoboxesWithPushPins handler if any
                                    if (infoboxesWithPushPins[infoboxWithPushPinIndex].addHandler) {
                                        that.setInfoboxesWithPushPinsHandler(infobox[infoboxCreateType][infoboxWithPushPinIndex], _pushpin2, infoboxesWithPushPins[infoboxWithPushPinIndex].addHandler);
                                    } 
                                }
                            } 
                        } 
                    }
                    //Create a ClusterLayer and add it to the map. Only if coordinates exist
					if (locs.length>0 &&
						infoboxesWithPushPins &&
						infoboxesWithPushPins.length > 0 &&
						infoboxesWithPushPins[0].viewChanged === false) {
                        // let clusterLayer = new Microsoft.Maps.ClusterLayer(clusterLayerArr, { clusteredPinCallback: that.getClusteredPinsInfo });
                        // map[mapReference].layers.insert(clusterLayer);
                        //Set view so that all pins are visible
                        map[mapReference].setView({
            ...
            key: 'getClusteredPinsInfo',
            value: function getClusteredPinsInfo(cluster) {
                if (this.state.clusterCallback) {
                    Microsoft.Maps.Events.addHandler(cluster, "click", this.state.clusterCallback);
                }
            }
        }, {
		key: 'setGetLocation',
		...
		key: 'setAutoSuggest',
		value: function setAutoSuggest(autoSuggest, mapReference) {
			let that = this;
			if (map[mapReference] && autoSuggest) {
				var suggestionBoxId = autoSuggest.suggestionBoxId ? autoSuggest.suggestionBoxId : null;
				var suggestionContainerId = autoSuggest.suggestionContainerId ? autoSuggest.suggestionContainerId : null;
				var callbackEvent = autoSuggest.callback ? autoSuggest.callback : null;

				if (suggestionBoxId && suggestionContainerId && callbackEvent)
				{
					Microsoft.Maps.loadModule(['Microsoft.Maps.AutoSuggest', 'Microsoft.Maps.Search'], function () {
						...
						manager.attachAutosuggest(suggestionBoxId, suggestionContainerId, function (callback, e) {
							map[mapReference].setView({ bounds: e.bestView });
							that.MakeCallback(callback, e);
						}.bind(that, autoSuggest.callback));
					});
				}
			}
		}
...
ReactBingmaps.propTypes = {
	...
	autoSuggest: _propTypes2.default.shape({
		suggestionBoxId: _propTypes2.default.string,
		suggestionContainerId: _propTypes2.default.string,
		callback: _propTypes2.default.func
	}),
    ...
    clusterClickCallback: _propTypes2.default.func
};
ReactBingmaps.defaultProps = {
    ...
    clusterClickCallback: undefined
};
